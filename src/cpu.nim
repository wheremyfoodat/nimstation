...
proc set_sideload*(emusideload_file: string) =
    sideload = true
    sideload_file = emusideload_file
...
proc decode_and_execute(instruction: uint32) =
    ...
    t = (instruction shr 16) and 0x1F
    imm = instruction and 0xFFFF
    s = (instruction shr 21) and 0x1F
    d = (instruction shr 11) and 0x1F
    subfunction = instruction and 0x3F
    shift = (instruction shr 6) and 0x1F
...
proc run_next_instruction*() =
    if (pc == 0x80030000'u32) and sideload:
